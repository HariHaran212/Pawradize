  ...
  if (!pet) {
    return (
      <div className="text-center py-20 font-poppins">
        <h2 className="text-2xl font-bold text-primary">Pet not found!</h2>
        ...
        <div className="grid md:grid-cols-2 gap-10 bg-white p-8 rounded-3xl shadow-lg shadow-accent/50">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-wrap gap-2 my-4">
              ...
              <span className="text-xs font-semibold px-3 py-1 bg-ivory text-primary rounded-full">{pet.type}</span>
            </div>

            <h2 className="text-xl font-bold text-primary mt-4">About {pet.name}</h2>
            <p className="text-text-medium leading-relaxed mt-2 flex-grow">{pet.description}</p>

            <div className="mt-4">
              <h3 className="font-semibold text-text-dark">Good with:</h3>
              <div className="flex flex-wrap gap-2 mt-2">
                {pet.goodWith.map(item => <span key={item} className="text-sm px-3 py-1 bg-highlight text-text-dark rounded-full">{item}</span>)}
              </div>
            </div>
            ...
            <div className="mt-8 flex items-center space-x-4">
                ...
                <button onClick={() => setIsFavorited(!isFavorited)} className="p-3 border-2 border-red-500 rounded-lg text-red-500 hover:bg-red-100 transition-colors">
