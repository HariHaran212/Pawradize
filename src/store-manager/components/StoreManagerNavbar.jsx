// Located inside src/layouts/StoreManagerLayout.jsx or as a separate component
...
const StoreManagerNavbar = () => {
  ...
  const navLinks = [
    ...
    { name: 'Products', href: '/manager/products' },
  ];
  return (
    <header className="bg-white shadow-md sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-6 h-16 flex justify-between items-center">
        ...
        <div className="flex items-center gap-8">
            <Link to="/manager/dashboard" className="text-xl font-bold text-primary">Pawradise <span className="font-normal text-text-medium">Store Manager</span></Link>
        ...
        <div className="relative">
            ...
            {isProfileOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl py-2">
                    <Link to="/manager/profile" className="flex items-center gap-3 px-4 py-2 text-sm text-text-dark hover:bg-ivory">
                        <HiOutlineUserCircle /> My Profile
                    </Link>
                    <Link to="/manager/settings" className="flex items-center gap-3 px-4 py-2 text-sm text-text-dark hover:bg-ivory">
                        <HiOutlineCog /> Settings
                    </Link>
                    <hr className="my-2 border-accent" />
                    <Link to="/logout" className="flex items-center gap-3 px-4 py-2 text-sm text-red-500 hover:bg-red-50">
